...
export default function App() {
  ...
  const saveRestore = async (data, screenshotFile) => {
    try {
      const { id, created_at, ...fields } = data
      let saved
      if (id) { saved = await api.updateRestore(id, fields); setRestores((prev) => prev.map((r) => r.id === id ? saved : r)) }
      else { saved = await api.createRestore(fields); setRestores((prev) => [...prev, saved]) }
      if (screenshotFile) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a' }}>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        {view === 'devices' && <DevicesList devices={devices} restores={restores} onNew={() => setDeviceModal({})} onEdit={(d) => setDeviceModal(d)} onDelete={deleteDevice} onViewDevice={viewDevice} onLogRestore={logRestoreForDevice} />}
        {view === 'restores' && <RestoreLog restores={restores} devices={devices} search={search} setSearch={setSearch} filterStatus={filterStatus} setFilterStatus={setFilterStatus} filterClient={filterClient} setFilterClient={setFilterClient} onEdit={(r) => setRestoreModal(r)} onDelete={deleteRestore} onAdd={() => setRestoreModal({})} />}
